...
export default function CardsSection({ linkZone }) {
  ...
  const [contact, getUnread] = useContact()
  const [unreadMessages, setUnreadMessages] = useState(0)
  useEffect(() => {
    if(contact?.SmsList?.length > 0){
      const uMess = getUnread() 
      setUnreadMessages(uMess)
    }
  }, [contact])
  ...
  return (
    <div className="w-full flex flex-wrap justify-center items-center min-h-screen">
      ...
      <button className="absolute top-5 right-20" onClick={() => router.push('/sms')}>
        {unreadMessages > 0 && <span className="absolute -top-2 inline-flex items-center justify-center px-2 py-1 mr-2 text-xs font-bold leading-none text-red-100 bg-red-600 rounded-full">{unreadMessages}</span>}
